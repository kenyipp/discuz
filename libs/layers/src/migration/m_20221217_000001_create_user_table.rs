...
impl MigrationTrait for Migration {
	async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
		...
		manager.create_table(
			Table::create()
				.table(User::Table)
				.col(ColumnDef::new(User::UserId).string().string_len(64).not_null().primary_key())
				.col(ColumnDef::new(User::Sub).string().string_len(64).not_null())
				.col(ColumnDef::new(User::Name).string().string_len(150).not_null())
				.col(ColumnDef::new(User::Email).string().string_len(250).not_null().unique_key())
				.col(ColumnDef::new(User::Phone).string().string_len(150))
				.col(ColumnDef::new(User::AvatarUrl).text())
				.col(ColumnDef::new(User::Notes).text())
				.col(ColumnDef::new(User::StatusId).string().string_len(1).not_null().default("A"))
				.col(
