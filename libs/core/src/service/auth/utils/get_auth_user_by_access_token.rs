...
use crate::service::auth::{
	auth_service::AuthUser, errors::AuthError, provider::api_provider::ApiCognitoTrait,
};
...
pub async fn get_auth_user_by_access_token(
	...
	let user = api_provider
		...
		.await
		.change_context(AuthError::Generic(
			"Unable to retrieve user context by the access token".to_owned(),
		))?;
	let attributes = match user.user_attributes() {
		Some(attributes) => attributes.iter(),
		None => {
			return Err(Report::new(AuthError::Generic(
				"Unable to extract attributes".to_owned(),
			)));
		}
	};
	let response = AuthUser {
		...
		email_verified: get_value_by_key(&attributes, "email_verified")
			.map(|email_verified| email_verified == "true"),
		name: get_value_by_key(&attributes, "name").unwrap(),
		phone_number_verified: get_value_by_key(&attributes, "phone_number_verified")
			.map(|phone_number| phone_number == "true"),
...
fn get_value_by_key(attributes: &Iter<AttributeType>, key: &str) -> Option<String> {
	if let Some(attribute) = attributes.to_owned().find(|x| {
